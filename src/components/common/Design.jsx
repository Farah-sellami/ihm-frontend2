...
const PrimaryButton = ({ children, className }) => {
  return (
    <button type="submit" className={`${className} text-white bg-[#68c447] font-medium rounded-full text-lg px-16 py-3 hover:bg-[#cbdec5] transition ease-in-out`}>
      {children}
    </button>
  );
};
const SecondaryButton = ({ children, className }) => {
  return (
    <button type="submit" className={`${className} text-white bg-[#ED8200] font-medium rounded-full text-lg px-16 py-3 hover:bg-[#CC7000] transition ease-in-out`}>
      {children}
    </button>
  );
};
...
export { Title, Body, Caption, CustomLink, CustomNavLink, Container, PrimaryButton, SecondaryButton, ProfileCard, Heading, CustomNavLinkList };
